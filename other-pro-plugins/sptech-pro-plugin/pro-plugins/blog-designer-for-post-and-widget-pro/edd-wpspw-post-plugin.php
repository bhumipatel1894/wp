...
function wpspw_pro_license_menu() {
	add_submenu_page( 'edit.php', __('Blog Designer Plugin License', 'blog-designer-for-post-and-widget'), __('Blog Designer License', 'blog-designer-for-post-and-widget'), 'manage_options', 'wpspw-pro-license', 'wpspw_pro_license_page' );
}
add_action('admin_menu', 'wpspw_pro_license_menu', 30);
...
function wpspw_pro_license_page() {
	...
	$license_info 	= get_option( 'edd_wpspwpost_license_info' );
?>
	<div class="wrap">
		<h2><?php _e('Blog Designer Pro Plugin License'); ?></h2>
				...
				<tbody>
					<tr valign="top">
						<th scope="row" valign="top">
							<label for="edd_wpspwpost_license_key"><?php _e('License Key'); ?></label>
						</th>
						<td>
							<input id="edd_wpspwpost_license_key" name="edd_wpspwpost_license_key" type="text" class="regular-text" value="<?php esc_attr_e( $license ); ?>" /><br/>
...
function edd_spost_activate_license() {
	...
	if( isset( $_POST['edd_license_activate_wpspwpost'] ) ) {
		...
		if(isset($license_data->license)) {
			update_option( 'edd_wpspwpost_license_info', $license_data );
		}
	}
}

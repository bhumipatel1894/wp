<?php
/**
 * 'bestselling_products_slider' Shortcode
 * 
...
function wcpscwc_pro_bestselling_products_slider($atts, $content) {
	...
	$exclude_post 			= (!empty($exclude_post))				? explode(',',$exclude_post)	: '';
	...
	// If product is there
	if ( $products->have_posts() ) : if($design == 'default'){
			?>
			...
			<?php 
			if( $active_slider ) { ?>
				<div class="wcpscwc-slider-conf" data-conf="<?php echo htmlspecialchars(json_encode($slider_conf)); ?>"></div><!-- end of-slider-conf -->
			<?php } ?>
				</div>
			</div>
			<?php
...
// 'bestselling_products_slider' shortcode
add_shortcode( 'bestselling_products_slider', 'wcpscwc_pro_bestselling_products_slider' );
add_shortcode( 'wcpscwc_bestselling_pdt_slider', 'wcpscwc_pro_bestselling_products_slider' );
...
class Wpbawm_Admin {
	...
	function wpbawm_pro_plugin_row_meta( $links, $file ) {
		...
		if ( $file == WPBAWM_PLUGIN_BASENAME ) {
			...
			$row_meta = array(
				'docs'    => '<a href="' . esc_url('https://www.wponlinesupport.com/pro-plugin-document/document-wp-blog-and-widget-masonry-add-on/') . '" title="' . esc_attr( __( 'View Documentation', 'wp-blog-and-widgets' ) ) . '" target="_blank">' . __( 'Docs', 'wp-blog-and-widgets' ) . '</a>',
	...
	function wpbawm_pro_plugin_action_links( $links ) {
		...
		$license_url = add_query_arg( array( 'post_type' => WPBAWM_POST_TYPE, 'page' => 'wpbawm-license'), admin_url('edit.php') );

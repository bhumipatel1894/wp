...
if( !defined( 'WPBAWM_VERSION' ) ) {
    define( 'WPBAWM_VERSION', '1.0.1' ); // Version of plugin
}
...
function wpbawm_admin_notices() {
	...
	if ( !wpbawm_plugins_active() ) {
		...
		echo sprintf( __('<p><strong><a href="%s" target="_blank">%s</a> </strong> OR <strong><a href="%s" target="_blank">%s</a></strong></p>', 'wp-blog-and-widgets'), 'https://wordpress.org/plugins/wp-blog-and-widgets/', 'WP Blog and Widget (Free)', 'https://www.wponlinesupport.com/wp-plugin/wp-blog-and-widgets/', 'WP Blog and Widgets' );
		echo '</div>';
	}
}

/***** Updater Code Starts *****/
define( 'EDD_WPBAWM_STORE_URL', 'https://www.wponlinesupport.com' );

...
class Wpbaw_Pro_Admin {
	...
	function __construct() {
		...
		add_action( 'restrict_manage_posts', array($this, 'wpbaw_pro_add_post_filters'), 50 );
		...
		// Action to add custom col to blog listing  ** testing 
		add_filter( 'manage_'.WPBAW_PRO_POST_TYPE.'_posts_columns', array($this, 'wpbaw_pro_posts_columns_try'));
	...
	function wpbaw_pro_register_settings() {
		register_setting( 'wpbaw_pro_plugin_options', 'wpbaw_pro_options', array($this, 'wpbaw_pro_validate_options') ); // first name is of register field name
	}
	...
	/**
	 * Add category dropdown to blog listing page
	 * 
	 * @package WP Blog and Widgets Pro
	 * @since 1.1.9
	 */
	function wpbaw_pro_add_post_filters() {

		global $typenow;
		
		if( $typenow == WPBAW_PRO_POST_TYPE ) {

			$wpbaw_cat = isset($_GET[WPBAW_PRO_CAT]) ? $_GET[WPBAW_PRO_CAT] : '';

			$dropdown_options = apply_filters('wp_tsasp_cat_filter_args', array(
					'show_option_all' 	=> __('All Blog Categories', 'wp-team-showcase-and-slider'),
					'hide_empty' 		=> 1,
					'hierarchical' 		=> 1,
					'show_count' 		=> 0,
					'orderby' 			=> 'name',
	...
	// self testing
	function wpbaw_pro_posts_columns_try($columns){
		$new_columns['wpbaw_pro_order1'] = __('Custom test','wp-blog-and-widgets1');

		$columns = wpbaw_pro_add_array( $columns, $new_columns ,5 );
	...
	function wpbaw_pro_plugin_row_meta( $links, $file ) {
		...
		if ( $file == WPBAW_PRO_PLUGIN_BASENAME ) {
			...
			$row_meta = array(
				'docs'    => '<a href="' . esc_url('https://www.wponlinesupport.com/pro-plugin-document/document-wp-blog-and-widgets-pro/') . '" title="' . esc_attr( __( 'View Documentation', 'wp-blog-and-widgets' ) ) . '" target="_blank">' . __( 'Docs', 'wp-blog-and-widgets' ) . '</a>',
				'support' => '<a href="' . esc_url('https://www.wponlinesupport.com/welcome-wp-online-support-forum/') . '" title="' . esc_attr( __( 'Visit Customer Support Forum', 'wp-blog-and-widgets' ) ) . '" target="_blank">' . __( 'Support', 'wp-blog-and-widgets' ) . '</a>',
			);

...
function edd_news_license_menu() {
	add_submenu_page( 'edit.php?post_type='.WPNW_PRO_POST_TYPE, __('News Plugin License', 'sp-news-and-widget'), __('Plugin License', 'sp-news-and-widget'), 'manage_options', 'newspro-license', 'edd_news_license_page' );
}
add_action('admin_menu', 'edd_news_license_menu', 20);
				...
				<tbody>
					<tr valign="top">
						<th scope="row" valign="top">
							<label for="edd_news_license_key"><?php _e('License Key'); ?></label>
						</th>
						<td>
							<input id="edd_news_license_key" name="edd_news_license_key" type="text" class="regular-text" value="<?php esc_attr_e( $license ); ?>" /><br/>
...
function edd_news_activate_license() {
	...
	if( isset( $_POST['edd_license_activate_news'] ) ) {
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'item_name_mismatch' :
						...
						$message = sprintf( __( 'This appears to be an invalid license key for %s.' ), EDD_NEWS_ITEM_NAME );
						break;
		...
		if ( ! empty( $message ) ) {
			$redirect = add_query_arg( array( 'sl_activation' => 'false', 'message' => urlencode( $message ) ), WPNW_PRO_LICENSE_URL );
		...
		wp_redirect( WPNW_PRO_LICENSE_URL );
		exit();
	}
}
 ...
 * @since 1.0.0
 */
function wpnw_pro_license_admin_notices() {
...
add_action( 'admin_notices', 'wpnw_pro_license_admin_notices' );
...
function wpnw_pro_plugin_row_license_missing( $plugin_data, $version_info ) {
	...
	$license 		= get_option( 'edd_news_license_info' );
	$license_status = get_option( 'edd_news_license_status' );

	if( ( !is_object($license) || $license_status !== 'valid' ) ) {
		echo '&nbsp;<strong><a href="' . esc_url( WPNW_PRO_LICENSE_URL ) . '">' . __( 'Enter valid license key for automatic updates.', 'sp-news-and-widget' ) . '</a></strong>';
	}
}

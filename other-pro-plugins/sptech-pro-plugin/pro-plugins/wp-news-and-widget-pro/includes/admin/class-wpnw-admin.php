...
class Wpnw_Pro_Admin {
	...
	function wpnw_pro_validate_options( $input ) {
		...
		$input['custom_css'] 	= isset($input['custom_css']) 	? wpnw_pro_slashes_deep($input['custom_css'], true) : '';
	...
	function wpnw_pro_add_post_row_data( $actions, $post ) {
		...
		if( $post->post_type == WPNW_PRO_POST_TYPE ) {
			return array_merge( array( 'wpnw_pro_id' => 'ID: ' . $post->ID ), $actions );
		}
	...
	function wpnw_pro_plugin_row_meta( $links, $file ) {
		...
		if ( $file == WPNW_PRO_PLUGIN_BASENAME ) {
			...
			$row_meta = array(
				'docs'    => '<a href="' . esc_url('https://www.wponlinesupport.com/pro-plugin-document/document-wp-news-and-scrolling-widgets-pro') . '" title="' . esc_attr( __( 'View Documentation', 'sp-news-and-widget' ) ) . '" target="_blank">' . __( 'Docs', 'sp-news-and-widget' ) . '</a>',
				'support' => '<a href="' . esc_url('https://www.wponlinesupport.com/welcome-wp-online-support-forum/') . '" title="' . esc_attr( __( 'Visit Customer Support Forum', 'sp-news-and-widget' ) ) . '" target="_blank">' . __( 'Support', 'sp-news-and-widget' ) . '</a>',
			);
